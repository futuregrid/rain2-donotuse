...
class ImgStoreMongo(AbstractImgStore):
    ...
    # addRequestItem
    ############################################################
    def addRequestItem(self, imageEntry1, requestInstance) :
        #self._items.append(imgEntry)
        status = False
        status = self.persistToStoreFromRequest(imageEntry1,requestInstance)
        return status
    ...
    ############################################################
    # persistToStoreFromRequest
    ############################################################
    def persistToStoreFromRequest(self, item, requestBody) :
        ...
        if (self.mongoConnection()):
            try:
                ...
                imgId = gridfsLink.put(requestBody.file, chunksize=4096 * 1024)
    ...
    def mongoConnection(self):
        ...
        try:
            ...
            connected = True
            print "Connection address " + self._mongoAddress
