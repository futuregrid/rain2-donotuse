    ...
    #XCAT
        ...
        if (operatingsystem!="ubuntu"):
            ...
            if(TEST_MODE):
                ...
                os.system('echo "172.29.200.1 t1 tm1" >> '+tempdir+'/rootimg/etc/hosts')
                os.system('echo "172.29.200.3 tc1" >> '+tempdir+'/rootimg/etc/hosts')
                os.system('echo "149.165.145.35 tc1r.tidp.iu.futuregrid.org tc1r" >> '+tempdir+'/rootimg/etc/hosts')
                ...
                logger.info('Configuring network')                   
                runCmd('wget ' + base_url + '/conf/centos/ifcfg-eth1 -O '+tempdir+'/rootimg/etc/sysconfig/network-scripts/ifcfg-eth1')
                runCmd('wget ' + base_url + '/conf/hosts_india -O '+tempdir+ '/rootimg/etc/hosts')
                #temporal
                runCmd('mkdir -p '+tempdir+'/rootimg/root/.ssh')
