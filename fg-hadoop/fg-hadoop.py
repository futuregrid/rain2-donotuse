...
def generate_job_script(job_name,hadoop_command, data_input_dir, data_output_dir, walltime,queue,num_nodes=1) :
    ...
    masters_file.write("python $FG_HADOOP_HOME/generate-xml.py $PBS_NODEFILE "+local_storage_dir+" "+hadoop_conf_dir+" \n\n") 
    ...
    if(data_input_dir):
        masters_file.write("$HADOOP_HOME/bin/hadoop fs -put "+ data_input_dir +" input \n") 
    ...
    if(data_output_dir):
        masters_file.write("$HADOOP_HOME/bin/hadoop fs -get output "+ data_output_dir +" \n") 
...
subprocess.call("export FG_HADOOP_HOME="+sys.path[0], shell=True)
generate_job_script("test1", "jar wordcount.jar WordCount input output", "~/input", "~/output", "0:20:00","")
