...
class fgUtil(object):
    ...
    def loadConfig(self):
        ...
        try:
            self._loghistdir=os.path.expanduser(config.get('LogHist', 'loghistdir', 0))        
        ...
        ##Log
        try:
            self._logLevel=string.upper(config.get('LogHist', 'log_level', 0))
        except ConfigParser.NoOptionError:
            self._logLevel=self._defaultLogLevel
            
        if not (self._logLevel in self._logType):
            print "Log level "+self._log_level+" not supported. Using the default one "+self._defaultLogLevel
            self._logLevel=eval("logging."+self._defaultLogLevel)
        else:
            self._logLevel=eval("logging."+self._logLevel)
